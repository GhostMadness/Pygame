...
def start_location_4():
    ...
    while running:
        ...
        if pygame.sprite.spritecollideany(gg_4, men_group) and key[pygame.K_RETURN]:
            menshik.dialog(
                "Вообщем я так понял NТы быздомный? NЯ предлагаю тебе работу. NИди и убей NВсех призраков в округе. NВзамен я позволю тебе жить NУ меня дома! NНа этом все NМне нужно работать NУдачи путник!")
